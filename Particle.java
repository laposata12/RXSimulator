...
public class Particle {
    ...
    private double mass = 10 * Math.pow(10, -14);
    ...
    public void chargeAccelerateSingle(Particle p){
        try{
            double force =charge * p.getCharge();
            force /= Math.pow(getDist(p),2);
            force *= ELECTRIC_CONSTANT;
    ...
    public void magneticAccelerate(Particle p){
        try{
            double[] velocity  = new double[]{xVelocity, yVelocity, zVelocity};
            double[] r = getDistComponents(p);
            double[] crossed = crossProduct(velocity, r);
            double[] pVelocity = p.getVelocityComponents();
            crossed = crossProduct(pVelocity, crossed);
            double force = MAGNETIC_CONSTANT / (Math.PI * 4);
            force *= charge;
            force *= p.getCharge();
            force /= Math.pow(getDist(p), 3);
            crossed = scaleArray(crossed, force);
            double[] opp = scaleArray(crossed,-1);
            forceAccelerate(crossed);
            p.forceAccelerate(opp);
        }catch(Exception e){
            System.out.println("No particle");
        }
    }
    public void rigMagnetic(){
        double[] velocity  = new double[]{xVelocity, yVelocity, zVelocity};
//		System.out.println("x "+xVelocity+" y "+yVelocity+" z "+zVelocity);
        velocity = scaleArray(velocity, charge);
        double[] bField = new double[]{0, 0, -.0000005};
        double[] crossed = crossProduct(velocity, bField);
//		printArray(crossed);
        forceAccelerate(crossed);
    }
